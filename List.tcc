...
namespace List_NS
{
    ...
    List<T>::List(std::initializer_list<T> lst)
        : List{}
    {
        for ( auto const& val : lst )       // Godtyckligt stor datatyp därav const&
        {
            push_back(val);
        }
    }
    ...
    template<typename T>
    void List<T>::push_front(T const& value) // Godtyckligt stor datatyp därav const&
    {
        Node * old_first { head->next.get() };
        head->next = std::make_unique<Node>(value, head.get(), head->next.release());   // uniqueptr istället för release
